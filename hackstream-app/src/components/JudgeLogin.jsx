...
const handleSubmit = async (e) => {
  ...
  try {
    ...
    if (!res.ok) {
      setServerError(data.error || "Invalid login");
      return;
    }

    // ✅ SUCCESS: Your backend returns { "message": "...", "name": "..." }
    // We save this as 'judgeUser'
    localStorage.setItem("judgeUser", JSON.stringify(data));
    
    // Redirect to Judge Dashboard
    navigate("/judge");

  } catch (err) {
    // This is where you are seeing "Server not responding" 
    // because of a typo or logic error above
